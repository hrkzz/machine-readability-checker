...
st.markdown(
    "チェックしたいファイルのシート名と表頭（ヘッダー）の範囲を指定してください。"
)

# --- 1. ファイルアップロード ---
uploaded_file = st.file_uploader(
    "CSV または Excel ファイルをアップロード", type=["csv", "xlsx", "xls"]
)
...
if uploaded_file is not None:
    ...
    with col1:
        selected_sheet = st.selectbox(
            "対象シートの選択", st.session_state["sheet_names"], key="selected_sheet"
        )
        ...
        try:
            with st.spinner("構造解析中..."):
                ...
                if file_suffix == ".xls" or file_suffix == ".csv":
                    ...
                    wb = load_workbook(
                        st.session_state["uploaded_path"], data_only=True
                    )
...
if ctx is not None and st.session_state.get("structure_done"):
    ...
    if st.session_state.get("check_done") is not True and st.session_state.get(
        "structure_done"
    ):
        ...
        with st.spinner("チェック中..."):
            ...
            for i, rule in enumerate(rules):
                ...
                progress.progress(
                    progress_val,
                    text=f"LEVEL1 チェック中: {rule.get('id', '')} - {rule.get('description', '')}...",
                )

                if fn is None:
                    passed, msg = (
                        False,
                        f"エラー発生: 対応する関数 '{fn_name}' が level1_checker に見つかりません。",
                    )
                else:
                    try:
            ...
            summary_md += "\n".join(table_lines)
            overall_comment = "レベル1のチェックが完了しました。詳細は下の「詳細」セクションを参照してください。"
            ...
            st.session_state["overall_comment"] = overall_comment
            st.session_state["check_done"] = True
            st.rerun()  # 結果表示のため再実行
...
if (
    ...
    and st.session_state.get("check_done")
):
    st.markdown("---")
    st.header("✅ 診断結果")
    ...
    overall_comment = st.session_state["overall_comment"]
    ...
    for level, checks in results:
        ...
        with st.expander(f"**{level.upper()} チェックの詳細**"):
            for item in checks:
                ...
                st.markdown(
                    f"- 判定: {'**合格**' if item['result'] == '✓' else '**不合格**'}"
                )
    ...
    report_filename = (
        f"{Path(file_name).stem}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    )
