...
from src.checker.utils.xlsx_utils import (
...
class XLSXLevel1Checker(BaseChecker):
    """
    XLSX専用のLevel1チェッカー
    """
    ...
    def __init__(self):
        super().__init__()
        self.logger.add("logs/xlsx_checker1.log", rotation="10 MB", retention="30 days", level="DEBUG")
    
    def get_supported_file_types(self) -> set:
        return {".xlsx"}
    
    def check_valid_file_format(self, ctx: TableContext, workbook: openpyxl.Workbook, filepath: str) -> Tuple[bool, str]:
        """XLSX固有のファイル形式チェック"""
    ...
    def check_no_images_or_objects(self, ctx: TableContext, workbook: openpyxl.Workbook, filepath: str) -> Tuple[bool, str]:
        """XLSX固有のオブジェクトチェック"""
        path = Path(filepath)
    ...
    def check_one_table_per_sheet(self, ctx: TableContext, workbook: openpyxl.Workbook, filepath: str) -> Tuple[bool, str]:
        """XLSX固有の複数テーブルチェック"""
        if workbook is None:
    ...
    def check_no_hidden_rows_or_columns(self, ctx: TableContext, workbook: openpyxl.Workbook, filepath: str) -> Tuple[bool, str]:
        """XLSX固有の非表示行・列チェック"""
        if workbook is None:
            return False, "ワークブック情報が不足しているためチェックできません"
    ...
    def check_no_notes_outside_table(self, ctx: TableContext, workbook: openpyxl.Workbook, filepath: str) -> Tuple[bool, str]:
        """XLSX固有の表外注釈チェック"""
    ...
    def check_no_merged_cells(self, ctx: TableContext, workbook: openpyxl.Workbook, filepath: str) -> Tuple[bool, str]:
        """XLSX固有の結合セルチェック"""
        if workbook is None:
            return False, "ワークブック情報が不足しているためチェックできません"
    ...
    def check_no_format_based_semantics(self, ctx: TableContext, workbook: openpyxl.Workbook, filepath: str) -> Tuple[bool, str]:
        """XLSX固有の書式チェック"""
        if workbook is None:
            return False, "ワークブック情報が不足しているためチェックできません"
    ...
    def check_no_whitespace_formatting(self, ctx: TableContext, workbook: openpyxl.Workbook, filepath: str) -> Tuple[bool, str]:
        """XLSX固有の空白チェック"""
        if workbook is None:
            return False, "ワークブック情報が不足しているためチェックできません"
        ...
        prompt = f"""
            以下はExcel(.xlsx)のセル値の一部です。これらの中に、見た目を整える目的（位置揃え・スペース調整など）で
    ...
    def check_single_data_per_cell(self, ctx: TableContext, workbook: openpyxl.Workbook, filepath: str) -> Tuple[bool, str]:
        """XLSX固有の1セル1データチェック"""
    ...
    def check_no_platform_dependent_characters(self, ctx: TableContext, workbook: openpyxl.Workbook, filepath: str) -> Tuple[bool, str]:
        """XLSX固有の機種依存文字チェック"""
        if workbook is None:
            return False, "ワークブック情報が不足しているためチェックできません"
