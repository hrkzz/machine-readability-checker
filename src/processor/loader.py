...
# --- LLM非依存の構造解析・抽出関数 ---
def extract_structured_table_no_llm(
    info: Dict[str, Any], 
    header_start_row: int = 1, # 1-based: ヘッダー開始行
    header_end_row: int = 1    # 1-based: ヘッダー終了行
) -> TableContext:
    """
    LLMに依存せず、ユーザー定義の構造（ヘッダー範囲）に基づいてTableContextを構築する。
    ...
    # 2. 基本的な妥当性チェック
    if total_rows == 0 or header_start_idx < 0 or header_end_idx < header_start_idx or header_end_idx >= total_rows:
        logger.warning(f"データセットが小さすぎるか、ヘッダー指定が無効です。Total rows: {total_rows}")
        ...
        return TableContext(
            ...
            row_indices={"column_rows": [], "data_start": 0, "data_end": -1, "annotations": []}
        )
    ...
    # 4. 注釈行の抽出
    # 上部注釈（ヘッダー行より上）
    upper = df.iloc[:header_start_idx].copy() if header_start_idx > 0 else pd.DataFrame()
    lower = pd.DataFrame() 
    ...
    if header_row_count > 1:
        ...
        for level in arrays:
            ...
            last_val = ""
            for val in level:
                # 空白セルを上の行または左のセルの値で埋めるロジック（LLM時代のロジックを流用）
                if val == "":
                    val = last_val or "(空白)"
                else:
                    last_val = val
    ...
    if not data.empty:
        ...
        if len(cols) != data.shape[1]:
            logger.warning(f"ヘッダー列数({len(cols)})とデータ列数({data.shape[1]})が一致しません。")
...
def load_file_and_extract_context(
    ...
    try:
        ctx = extract_structured_table_no_llm(
            main_sheet, 
